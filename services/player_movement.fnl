...
(tset service ENUMS.p_state.moving
      (fn [entity track dt]
        ...
        (if (and (or (love.keyboard.isDown :up) (love.keyboard.isDown :w))
                 (< entity.vx max_move_amount) (not entity.input_pause?))
            (set entity.vx (+ entity.vx (* dt move_force)))
            (and (or (love.keyboard.isDown :down) (love.keyboard.isDown :s))
                 (> entity.vx (- max_move_amount)) (not entity.input_pause?))
            (set entity.vx (- entity.vx (* dt move_force)))
